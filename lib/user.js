    ...
    try {
        if(isValid) {
            ...
            if(email_verification) { 
                ...
                const url = `${base_url}/verify/${user_id.toString()}/${hash}`; //generate email url
...
export const updateUser = async(_id, payload) => {
    ...
    if(password) {
        const { salt, hash } = generateHash(payload.password);
        body = { ...body, salt, hash };
    }

    try {
        const target = { _id: new ObjectId(_id) };
        const { acknowledged, matchedCount, modifiedCount } = await db.collection("users").updateOne(target, {$set: body}); //update user
...
export const verifyHash = async(hash, _id) => {
    try {
        if(!hash) throw new Error('Must provide a hash...');
        if(!_id) throw new Error('Must provide an id...');
    
        const current_hash = await fetchEmailVerificationHash(hash);
